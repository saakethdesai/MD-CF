  ...
  Fix(lmp, narg, arg)
{
  ...
  lowimproper = force->numeric(FLERR,arg[7]);
  highimproper = force->numeric(FLERR,arg[8]);
  
  btype = force->inumeric(FLERR,arg[9]);
  ...
  if (highimproper < 0.0 || lowimproper < 0.0)
    error->all(FLERR,"Invalid bond type in fix bond/create command"); 
  ...
  int iarg = 10; //changed iarg to 10 so that it reads in both angles
  while (iarg < narg) 
  {
...
void FixBondCreate::init()
{
  ...
  // need a full neighbor list for checking both improper angles, built every Nevery steps
...
void FixBondCreate::post_integrate()
{
  ...
  int cid,pid,bid1,bid2; //current ID, partner ID, bond ID1, bond ID2
  double bond_energy, angle_value, dihedral_value, improper_value;
  ...
  for (ii = 0; ii < inum; ii++) 
  {
    ...
    for (jj = 0; jj < jnum; jj++)
    {
      ...
      for (int ll = 0; ll < nspecial[atom->map(id1)][0]-1; ll++) //-1 to get the bond atoms in one go
      {
      	id3 = special[atom->map(id1)][ll];
        id4 = special[atom->map(id1)][ll+1]; //requires the -1 above
        
        improper_value = compute_improper(id1,id3,id4,id2);
        if (improper_value >= lowimproper && improper_value <= highimproper) 
        {
        	possible = 0;
        }
        if (!possible) break;
...
double FixBondCreate::compute_improper(int id1, int id2, int id3, int id4)
{

  double **x = atom->x;
  int i1,i2,i3,i4,n,type;
  double vb1x,vb1y,vb1z,vb2x,vb2y,vb2z,vb3x,vb3y,vb3z;
  double eimproper, angle;
  ...
  id4 = domain->closest_image(id1,id4);
  id2 = domain->closest_image(id4,id2);
  id3 = domain->closest_image(id4,id3);
  ...
  angle = acos(c)*180/3.14159; //return in degrees
  return angle;
}
